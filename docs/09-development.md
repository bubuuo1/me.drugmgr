...
5. legacy 데이터 행 수와 약·일정·로그 스냅샷이 유지되는지 확인한다.
...
4. Production을 다시 배포한 뒤 Gmail과 다른 도메인의 주소로 각각 관리 요청을 보내 수신, 동일 이메일 Google 로그인, 수신자 소유 공간 선택·명시 동의와 요청자의 보호자 추가까지 확인한다. 수신자가 요청자 공간에 자동 추가되지 않는지도 확인한다.
...
- [x] 확인된 이메일 기반 가족 기록 관리 요청 생성·대상 공간 선택·명시 동의·거절·취소
...
| 기존 가족 초대가 요청자 공간을 상대에게 공유해 요청자가 상대 기록을 관리하려는 목적과 방향이 반대임 | 관리 요청 수신자가 자기 소유 공간을 선택하고 명시 동의하면 요청자가 해당 공간의 보호자가 되도록 UI·이메일·RPC·감사 필드를 통일했다. 수신자를 요청자 공간에 자동 추가하지 않고 기존 수락 관계도 자동 변환하지 않는다. |
...
- 확인된 이메일이 일치하는 관리 요청만 수락·거절하고 수신자 소유 공간 선택·명시 동의·단방향 보호자 생성 검증
...
- 복약 공간 선택과 역할 표시
- 가족 기록 관리 요청 생성·수신자 소유 공간 선택·명시 동의·수락·거절·취소와 반대 방향 멤버십 미생성
...
- 소유자·보호자의 약·일정 쓰기와 소유자 전용 공간·구성원·관리 요청 관리, 확인된 이메일·수신자 소유 공간·명시적 동의 기반 관리 요청 수락이 DB에서 강제됨
