...
4. `supabase/migrations/20260830060000_manage_invited_family_records.sql`을 적용해 가족 기록 관리 요청의 대상 공간 필드와 2인자 수락 RPC를 만든다. 반대 방향 의미였던 기존 대기 초대가 취소되고 기존 수락 관계는 변환되지 않았는지 확인한다.
5. `supabase/migrations/20260830070000_reciprocal_family_relationships.sql`을 적용해 새 요청의 양방향 수락, 기존 단방향 관계 표시·명시적 전환, 양쪽 관계 종료와 관계 접근 감사를 만든다. 이전 문구로 만들어진 대기 요청이 취소되고 기존 accepted 멤버십은 역방향 권한 없이 현재 한 방향으로만 연결되는지 확인한다.
6. `supabase/migrations/20260830070001_index_family_relationship_audit_foreign_keys.sql`을 적용해 관계 감사 외래키의 조회·참조 삭제 검사를 위한 부분 인덱스를 추가한다.
7. 앱 코드보다 먼저 `supabase/migrations/20260830080000_add_medication_schedule_outcomes.sql`을 적용한다. 신규 테이블·RLS·GRANT·복합 FK·trigger와 Push 함수 교체가 성공했는지 확인한다. 이 단계는 기존 테이블과 투약 로그를 바꾸지 않는 additive 변경이므로 이전 앱도 계속 동작하지만 새 일정 결과는 새 앱에서만 조회·작성한다.
8. legacy 데이터 행 수와 약·일정·로그 스냅샷이 유지되고 관계 및 일정 결과 마이그레이션이 기존 건강 기록을 변경하거나 삭제하지 않았는지 확인한다.
...
가족 기록 관리 요청 안내 메일은 Vercel 서버에서 Gmail SMTP로 발송한다. 운영 환경에는 `APP_BASE_URL`, `GMAIL_SMTP_USER`, `GMAIL_SMTP_APP_PASSWORD`와 선택 항목인 `GMAIL_SMTP_FROM_NAME`을 서버 전용 환경변수로 설정한다. Gmail 계정에는 2단계 인증과 앱 비밀번호가 필요하며 실제 비밀번호나 앱 비밀번호를 Git, `NEXT_PUBLIC_*`, 문서의 예시 값에 기록하지 않는다. DB RPC가 같은 요청의 발송을 1분에 한 번, 수신 주소별 하루 5회, 발신 사용자별 하루 50회, 앱 전체 하루 400회로 제한하며 하루는 한국 날짜를 기준으로 한다.
...
1. 관리 요청 발송에 사용할 Gmail 또는 Google Workspace 계정에서 2단계 인증을 켠다.
...
4. Production을 다시 배포한 뒤 Gmail과 다른 도메인의 주소로 각각 관리 요청을 보내 수신, 동일 이메일 Google 로그인, 수신자 소유 공간 선택·양방향 동의와 두 방향 보호자 추가까지 확인한다. 양쪽 가족 화면의 관계·공간 표시, 기록 대상 전환, 어느 쪽에서든 관계 종료와 두 접근 회수도 확인한다.
...
- [x] 확인된 이메일 기반 양방향 가족 기록 관리 요청 생성·대상 공간 선택·명시 동의·거절·취소
- [x] 기존 단방향 관계의 양쪽 표시·명시적 양방향 전환·어느 참여자의 종료와 관계 접근 감사
- [x] Gmail SMTP 관리 요청 안내 이메일 발송·재발송과 실패 상태 구분
- [x] 현재 복약 공간 선택과 공간 전환 시 데이터 격리
- [x] 명시적으로 선택한 가족 기록 대상의 URL 반영·내부 이동·새로고침 유지와 접근 회수 시 안전한 기본 공간 전환
...
- [x] 약 이름·단위·일정 시각 스냅샷
- [x] `medication_schedule_outcomes`의 두 결과 값, KST `scheduled_date`, 스냅샷·감사 필드와 메모
- [x] 일정 결과의 요청 ID unique, 일정·한국 날짜별 활성 행 부분 unique와 복합 FK
...
- [x] 일정 결과도 같은 요청 ID의 일정·날짜·결과·정규화한 메모가 같을 때만 멱등 성공
...
- [x] 일정 결과의 약·일정 스냅샷을 DB trigger가 채우고 일정 삭제 뒤에도 결과 의미 보존
- [x] 일정 결과의 `scheduled_date` 입력을 한국 달력 날짜로 검증
...
- [x] 기본 조회·저장 오류 UI
- [x] 모바일 기본 접근성
- [x] 최초 로딩과 백그라운드 재확인을 분리해 기존 화면과 입력 상태 유지
- [x] 포커스·표시 상태 복귀 즉시 전체 재조회, 60초 권한·초대 경량 확인과 5분 전체 안전 재조회
- [x] 홈의 첫 콘텐츠로 `오늘 예정`을 표시하고 마지막에 권한별 `약·일정 관리` 또는 보기 진입점 제공
...
- [x] 약별 오늘 누적
- [x] 완료 일정은 `기록 있음`, 미완료 일정은 권한별 `기록하기`로 표시하고 불필요한 미완료 상태 문구 생략
- [x] 실제 투약과 명시적 `복용하지 않음`·`약 없음` 일정 결과를 별도 상태로 표시
- [x] 기록 부재에 따른 자동 미복용·지연 판정 문구 없음
...
- [x] 메모
- [x] 일정 복용과 추가 복용 구분
- [x] 예정 일정에서 `복용하지 않음`·`약 없음`을 사용자가 직접 선택해 실제 투약과 분리 저장
...
- [x] 320px·375px·393px 화면과 날짜·시간 선택기 핵심 흐름 E2E
...
- [x] 같은 일정·한국 날짜의 명시적 일정 결과 저장 뒤 신규·점유된 반복 알림 중단
...
| 단방향 가족 공유로는 서로의 복약 기록을 함께 관리할 수 없음 | 새 요청은 수신자가 자기 소유 공간을 선택하고 양방향 권한에 동의하면 요청자와 수신자가 각각 상대 공간의 보호자가 되도록 UI·이메일·RPC·감사 필드를 통일했다. 기존 accepted 관계는 반대 방향 권한을 소급 생성하지 않고 양쪽 화면에 실제 방향으로 표시하며, 현재 상대 기록을 관리하는 참여자의 별도 공간 선택·동의로 전환하거나 어느 참여자든 종료할 수 있다. |
| `복용기록`·`환경설정` 탭 진입 시 내비게이션과 같은 상단 제목이 반복됨 | 두 탭의 보이는 제목을 제거하고 문서 구조와 화면 낭독용 `h1`만 유지했다. |
...
| 반복 알림을 멈추려면 실제 복용이 아닌데도 투약 로그를 만들 위험이 있음 | 예정 일정에 사용자가 직접 선택하는 `복용하지 않음`·`약 없음` 결과를 별도 테이블에 저장한다. 실제 복용 합계·마지막 복용에는 포함하지 않고 같은 일정·한국 날짜의 반복 알림만 중단한다. 기록 부재를 결과로 자동 판정하지 않는다. |
...
- mock E2E에서는 Supabase 경로를 사용하지 않으므로 60초 권한·초대 확인, 5분 전체 재조회와 포커스 복귀 재조회는 실제 계정·네트워크 통합 테스트가 남아 있다.
- 공간 초기 조회와 5분 백그라운드 재조회가 전체 삭제되지 않은 로그와 상태를 가져온 뒤 화면에서 날짜를 거른다. 홈·날짜별 서버 범위 조회가 남아 있다.
...
- 사용자별 private Broadcast 기반 실시간 무효화는 일반 Postgres Changes의 DELETE 격리 한계를 피하고 두 계정 관계 종료 E2E를 마련한 뒤 검토한다.
- 소유권 이전, 소유자 계정 상실, 가족 관계와 무관한 일반 멤버십의 자진 탈퇴, 공간·계정 삭제와 운영 백업 보존 기간은 별도 데이터 생명주기 결정이 필요하다. 수락된 가족 관계의 참여자 종료는 현재 구현 범위다.
...
- 확인된 이메일이 일치하는 관리 요청만 수락·거절하고 수신자 소유 공간 선택·양방향 동의·두 보호자 접근의 원자적 생성 검증
- 기존 accepted 관계의 실제 단방향만 보존, 양쪽 최소 관계 조회, 현재 관리자만 명시적으로 역방향 공간을 공유하는 전환 검증
- 양쪽 참여자의 관계 종료, 양방향 두 접근과 단방향 한 접근 회수, 기록 불변, 제3자 종료와 활성 관계 멤버십의 일반 한쪽 변경·제거를 RPC·트리거에서 거부
- 공간 전환 시 이전 공간 데이터가 남지 않음
- 사용자·기기·공간 Push opt-in과 멤버십 회수 후 발송 제외
- KST 날짜 변환과 자정 경계
- 수량 검증
- 일정 연결과 추가 복용 분류
- 일정 결과 두 값, KST 날짜, 실제 투약과 분리된 합계·마지막 복용 계산
- 일정 결과 스냅샷 trigger, 같은 일정·날짜 활성 행 unique, 복합 FK와 역할별 RLS
- 일정 결과의 같은 요청 ID 멱등 성공과 다른 payload 충돌
- 실제 투약 로그 또는 명시적 일정 결과 뒤 Push 신규 claim·발송 직전 재검증 제외
...
- 홈의 주요 동작
- 투약 기록 성공과 중복 제출 방지
- 예정 일정의 `복용하지 않음`·`약 없음` 저장, 홈·날짜별 기록 표시와 실제 복용 합계 불변
- 저장 실패 시 완료 처리 금지
- 상태 저장
- 날짜 이동과 기록 조회
...
- 복약 공간 선택과 역할 표시, 가족 기록 대상의 내부 이동·새로고침 유지와 해당 공간 데이터 재조회
- 양방향 가족 기록 관리 요청 생성·수신자 소유 공간 선택·명시 동의·수락·거절·취소와 두 방향 보호자 생성
- 수락 관계의 양쪽 표시·기록 대상 전환, 기존 단방향 표시·명시적 양방향 전환과 어느 참여자의 관계 종료
- 가족 기록 관리 요청 안내 이메일 발송 성공·실패와 재발송
...
- 소유자·보호자의 약·일정 쓰기와 소유자 전용 공간·구성원·관리 요청 관리, 확인된 이메일·양쪽 소유 공간·명시적 양방향 동의 기반 관리 요청 수락이 DB에서 강제됨
- 기존 단방향 관계는 소급 없이 실제 방향으로만 보존되고, 참여자 전환·종료와 양방향 접근 동시 회수가 DB에서 강제됨
- 실패한 쓰기를 성공으로 표시하지 않음
- 반복 제출로 중복 로그가 생기지 않음
- 명시적 일정 결과가 실제 투약과 분리되고 같은 일정·한국 날짜에 중복되지 않음
- 한국 날짜 경계와 과거 스냅샷 테스트 통과
- push 서비스 워커가 앱/API 요청을 오프라인 캐시하지 않음
- 발송 API, Supabase 발송 RPC와 Cron 비밀값이 브라우저나 Git 저장소에 노출되지 않음
- 실제 투약 또는 명시적 일정 결과가 있거나 비활성·삭제된 일정과 중복 발송이 DB 규칙 및 발송 직전 재검증으로 제외됨
- 기록 전 5분 회차와 한국 날짜 자정 종료가 DB 규칙으로 계산됨
- 관리 요청 메일 자격 증명이 브라우저나 Git 저장소에 노출되지 않고, 메일 링크만으로 가족 권한이 생기지 않음
...
- 백업·복원, CSV/PDF
- 기록 부재를 근거로 한 자동 미복용·지연 복용 판정과 알림의 정시·필수 도착 보장
- 통계·그래프·추이
- AI 분석과 의료 판단
