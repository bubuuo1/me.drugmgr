...
가족 기록 관리 요청 메일의 `GMAIL_SMTP_USER`, `GMAIL_SMTP_APP_PASSWORD`와 선택 항목인 `GMAIL_SMTP_FROM_NAME`도 Vercel 서버 전용 환경변수다. 앱 비밀번호는 코드, Git, 로그, `NEXT_PUBLIC_*`에 넣지 않는다. 메일 링크는 가족 권한 토큰이 아니다. 로그인 세션의 확인된 이메일과 대기 요청이 일치한 사용자가 자신이 소유한 복약 공간을 직접 선택하고 보호자 권한 공유에 명시적으로 동의해야만 DB 멤버십을 만든다. 생성 RPC는 자기 이메일과 보호자가 아닌 역할을 거부한다. 수락 RPC는 수신자의 대상 공간 소유권과 요청자의 현재 발신 자격을 다시 검사하고 요청자만 대상 공간에 추가하며, 수신자를 요청자의 공간에 추가하거나 기록 대상을 추론하지 않는다. 받은 요청 목록에는 처리에 필요 없는 요청자 공간 ID·이름을 반환하지 않는다. 발송 quota RPC는 서버 전용 `PUSH_DISPATCH_SECRET`을 검증하고 익명·이메일 미확인 사용자를 거부한다. 발송 권한은 DB가 소유자와 대기 요청을 다시 확인하고, 같은 요청은 1분에 한 번, 수신 주소별 하루 5회, 발신 사용자별 하루 50회, 앱 전체 하루 400회까지만 원자적으로 허용한다. 하루 기준은 한국 날짜다.
...
- 수신자가 자기 소유 공간을 선택해 동의한 경우에만 요청자가 보호자가 되고 반대 방향 멤버십은 생기지 않음
