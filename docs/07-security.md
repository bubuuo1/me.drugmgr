...
- `caregiver`: 소유자와 다른 구성원 정보를 포함한 전체 조회, 약·일정·투약 로그·명시적 일정 결과·하루 상태 작성·수정
- `viewer`: 조회 전용

소유자와 보호자는 약을 비활성화하거나 soft delete하고 일정을 삭제할 수 있다. 복약 공간 이름·구성원·초대 변경은 소유자만 할 수 있다. 본인이 참여한 가족 관계의 양방향 전환과 종료는 일반 구성원 관리 권한이 아니라 명시적 동의의 부여·철회이므로 참여자 확인 제한 RPC로 별도 처리한다.
...
가족 기록 관리 요청 메일의 `GMAIL_SMTP_USER`, `GMAIL_SMTP_APP_PASSWORD`와 선택 항목인 `GMAIL_SMTP_FROM_NAME`도 Vercel 서버 전용 환경변수다. 앱 비밀번호는 코드, Git, 로그, `NEXT_PUBLIC_*`에 넣지 않는다. 메일 링크는 가족 권한 토큰이 아니다. 로그인 세션의 확인된 이메일과 대기 요청이 일치한 사용자가 자신이 소유한 복약 공간을 직접 선택하고 양방향 보호자 권한에 명시적으로 동의해야만 DB 멤버십을 만든다. 생성 RPC는 자기 이메일, 양방향 동의 누락과 보호자가 아닌 역할을 거부한다. 수락 RPC는 수신자의 대상 공간 소유권과 요청자의 현재 발신 자격을 다시 검사하고 두 사용자를 각각 상대 소유 공간의 보호자로 원자적으로 추가한다. 받은 요청 목록에는 동의 전에 필요하지 않은 요청자 공간 ID·이름을 반환하지 않는다. 발송 quota RPC는 서버 전용 `PUSH_DISPATCH_SECRET`을 검증하고 익명·이메일 미확인 사용자를 거부한다. 발송 권한은 DB가 소유자와 대기 요청을 다시 확인하고, 같은 요청은 1분에 한 번, 수신 주소별 하루 5회, 발신 사용자별 하루 50회, 앱 전체 하루 400회까지만 원자적으로 허용한다. 하루 기준은 한국 날짜다.
...
- 투약 로그·명시적 일정 결과·하루 상태: 구성원은 조회, 소유자와 보호자만 생성·수정·삭제
- 가족 기록 관리 요청: 소유자용 발신 목록과 확인된 이메일 수신 목록, 제한 RPC로 관리
- 수락된 가족 관계: 두 참여자에게만 현재 접근 방향과 필요한 공간 이름을 최소 반환하고, 현재 상대 기록을 관리하는 참여자의 명시적 전환과 어느 참여자의 종료만 제한 RPC로 허용
...
- 자기 공간과 명시적으로 권한을 받은 공간의 허용 동작 성공
- 관계없는 공간의 목록·행·구성원·관리 요청 조회 실패
- 수신자가 자기 소유 공간을 선택해 양방향 권한에 동의한 경우에만 두 사용자가 각각 상대 공간의 보호자가 됨
- 기존 단방향 관계가 양쪽 참여자에게 실제 방향으로만 보이고 반대 방향 접근은 별도 공간 선택·동의 전까지 생기지 않음
- 관계없는 제3자의 관계 조회·전환·종료 실패와 두 참여자 각각의 종료 성공
- 양방향 종료 시 두 관계 접근이 함께 제거되고 단방향 종료 시 기존 접근만 제거되며 건강 기록 행은 유지됨
- 활성 관계가 만든 멤버십의 일반 한쪽 변경·제거가 RPC와 테이블 트리거 양쪽에서 실패
- 타인 소유 대상 공간, 확인되지 않거나 불일치하는 이메일, 대상 공간 없는 수락 거부
- 보호자의 약·일정 관리 성공과 공간 이름·구성원·초대 관리 실패
- 소유자·보호자의 일정 결과 생성 성공과 조회자의 생성 실패
- 조회자의 모든 쓰기 실패
- `care_space_id`와 약·일정 ID를 섞은 투약 로그·일정 결과 쓰기 실패

가족 관계 감사, Web Push 구독과 발송 이력은 Data API에 노출하지 않는 `private` 스키마에 두고 RLS를 활성화한다. `anon`과 `authenticated`에는 private 테이블 권한을 주지 않는다. 가족 관계 RPC는 `auth.uid()`가 두 참여자 중 한 명인지와 공간 소유권을 검사한다. Push 제한 RPC는 `auth.uid()`, endpoint 소유권, 선택 공간의 현재 멤버십과 Vault 발송 비밀값을 검사한다. 발송 직전에도 멤버십을 재확인해 관계 종료 등으로 권한이 회수된 사용자에게 새 알림을 보내지 않는다.

## 4. DB가 보장할 규칙

클라이언트 입력만 신뢰하지 않는다.

- 투약 로그와 일정 결과 각각의 중복 방지용 `(care_space_id, client_request_id)` unique
- 일정 결과의 `(care_space_id, schedule_id, scheduled_date)` 활성 행 부분 unique와 두 outcome 값 check
- 수량 양수 check
- 복약 공간+상태 날짜 unique
- 약·일정 FK
- 투약 로그와 일정 결과 스냅샷을 DB trigger로 생성
...
투약 로그와 일정 결과의 약 이름·단위·일정 시각 스냅샷은 클라이언트가 임의로 정하는 값이 아니라 insert 시 DB가 같은 복약 공간의 현재 설정에서 복사한다. 일정 결과는 사용자가 명시적으로 선택한 값만 저장하며 DB나 Push 함수가 기록 부재를 결과 행으로 자동 변환하지 않는다.
...
저장 대상은 로그인 식별자와 최소 프로필, 공유 초대와 관계·접근 감사 정보, 약 설정, 실제 복용 기록, 사용자가 명시한 일정 결과, 상태 선택, 사용자가 작성한 메모로 제한한다.

- Google에서 받은 표시 이름·아바타와 확인된 이메일 외에 주민등록번호, 주소, 전화번호를 요구하지 않는다.
- 확인된 이메일은 관리 요청 대상 일치 확인에만 사용하고 역할 판정에는 DB 멤버십을 사용한다.
...
- 로그아웃과 공유 권한 회수는 별개다. 활성 가족 관계는 어느 참여자든 가족 화면에서 종료하며, 양방향 관계의 두 보호자 접근은 함께 회수한다. 관계와 무관한 일반 구성원은 공간 소유자가 제거한다.
