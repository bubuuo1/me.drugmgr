...
- 가족 관리: 소유자가 Gmail·네이버 등 이메일 주소로 양방향 관리 요청을 보내고, 같은 이메일의 Google 계정으로 로그인한 상대가 자신의 복약 공간을 선택해 명시적으로 동의하면 두 사람이 서로 상대 공간의 보호자가 됨
...
새 사용자는 기록이 없는 개인 복약 공간을 받습니다. 다른 사람의 기록은 공간 소유자가 명시적으로 허용한 사람만 볼 수 있습니다. 새 관리 요청을 수락할 때 수신자는 자신이 소유한 공간을 직접 선택하고 양방향 보호자 권한에 동의합니다. 수락이 끝나면 요청자는 선택된 수신자 공간의 보호자가 되고 수신자는 요청자가 요청을 보낸 공간의 보호자가 됩니다. 두 공간과 기록은 합쳐지지 않으며 기록 대상을 전환해 따로 관리합니다.

이전 배포에서 이미 수락된 단방향 관계에는 반대 방향 권한을 자동으로 추가하지 않습니다. 두 사람의 가족 화면에 현재 접근 방향을 표시하고, 현재 상대 기록을 관리하는 사람이 자기 소유 공간을 선택해 별도로 동의한 경우에만 양방향으로 전환합니다. 어느 쪽이든 관계를 종료할 수 있으며 양방향 관계의 보호자 접근은 함께 제거되지만 저장된 약·일정·투약·상태 기록은 삭제되지 않습니다.
...
알림은 각 사용자가 현재 기기에서 필요한 복약 공간마다 직접 켜야 합니다. 가족 요청 수락이나 양방향 전환만으로 자동 구독되지 않으며, 공간 접근 권한이 해제되면 그 공간의 새 알림도 발송되지 않습니다. 활성 일정의 알림은 해당 한국 날짜의 같은 일정 기록이 생길 때까지 5분마다 발송을 시도합니다. 알림은 기록을 확인하라는 안내일 뿐 미복용 여부를 판정하거나 정시 도착을 보장하지 않습니다. iPhone/iPad는 iOS/iPadOS 16.4 이상에서 Safari로 홈 화면에 추가한 앱을 사용해야 합니다.
...
- 실제 사용자 두 명 이상으로 공간 격리, 관리 요청 이메일 일치, 수신자 소유 공간의 명시적 선택·양방향 동의, 두 방향 보호자 권한, 기존 단방향 관계의 비소급·명시적 전환, 양쪽 관계 종료와 조회자 쓰기 거부를 검증합니다.
